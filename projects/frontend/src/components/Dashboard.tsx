...
import { useClubs, useEvents, useDeleteClub } from "../hooks/useDashboard"
...
const Dashboard = () => {
  ...
  const { deleteClub } = (() => {
    const mutation = useDeleteClub()
    return { deleteClub: mutation }
  })()

  const handleDeleteClub = async (e: React.MouseEvent, clubId: string, clubName: string) => {
    e.stopPropagation()
  ...
  // Global loading for initial state (if both are loading)
  const isDashboardLoading = isClubsLoading || isEventsLoading
  ...
  }, [clubs, events, isDashboardLoading])
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col flex-shrink-0">
        ...
        <nav className="flex-1 p-4 overflow-y-auto">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between px-4 mb-3">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  title="Refresh Clubs"
                >
                  <span className={`text-xs ${isClubsFetching ? 'animate-spin' : ''}`}>🔄</span>
                </button>
                <button
            ...
            {(isClubsLoading || isClubsFetching) ? (
                ...
                {clubs.map((club: any) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg transition group ${activeSection === `club-${club.id}`
                        ? "bg-blue-50 text-blue-600"
                        : "text-gray-700 hover:bg-gray-50"
                        }`}
                    >
                      ...
                      {club.user_role === 'owner' && (
                        <div
                          onClick={(e) => handleDeleteClub(e, club.id, club.name)}
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between px-4 mb-3">
              ...
              <div className="flex items-center gap-1">
                ...
                >
                  <span className={`text-xs ${isEventsFetching ? 'animate-spin' : ''}`}>🔄</span>
                </button>
                <button
